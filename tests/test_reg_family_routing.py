...
def _x():
    n = int(np.prod(SHAPE))
    return np.arange(n, dtype=np.float64).reshape(SHAPE) / n
...
def _record_c_calls(monkeypatch, fn):
    """Run ``fn`` with the dlpack layer instrumented; return symbols hit."""
    seen = []
    names = [n for n in dir(_ff) if n.startswith(("field_", "flow_"))]
    for name in names:
        orig = getattr(_ff, name)

        def spy(*args, _n=name, _o=orig, **kwargs):
            seen.append(_n)
            return _o(*args, **kwargs)

        monkeypatch.setattr(_ff, name, spy)
    fn()
    return seen
...
def test_wrapper_only_calls_its_own_family(monkeypatch, name):
    ...
    seen = _record_c_calls(monkeypatch, _CASES[name])
